#!/bin/bash
# Level 2 Benchmark测试脚本 - 支持zero-shot和few-shot模式
# 用法: ./run_level2_test.sh [选项]
#
# 重要提示：
#   1. 请在使用前配置以下参数：
...
#      - 方式2: 通过命令行参数传入
#        ./run_level2_test.sh --api-key YOUR_KEY --base-url https://api.example.com/v1 --model MODEL_NAME

# 默认配置
MODEL="${LLM_MODEL_NAME:-YOUR_MODEL_NAME}"  # 请替换为您的模型名称，或通过--model参数传入
INPUT_FILE="ProOPF_B/level2_with_labels.jsonl"
EXAMPLE_FILE="ProOPF_D/level2_data_example.jsonl"
...
# 打印帮助信息
print_help() {
    echo "Level 2 Benchmark测试脚本"
    echo ""
    echo "用法: $0 [选项]"
    echo ""
    echo "选项:"
    echo "  --model MODEL           模型名称 (必需，请填入您要使用的模型名称)"
    echo "  --input-file FILE       输入文件路径 (默认: ProOPF_B/level2_with_labels.jsonl)"
    echo "  --example-file FILE     Few-shot示例文件路径 (默认: ProOPF_D/level2_data_example.jsonl)"
...
# 构建基础命令
BASE_CMD="python LLM_test/test_level2.py"
...
run_test() {
    local mode=$1
    local output_file="$OUTPUT_DIR/level2_test_${SAFE_MODEL_NAME}_${mode}_output.jsonl"
...
# 主执行逻辑
echo -e "${BLUE}========================================${NC}"
echo -e "${BLUE}Level 2 Benchmark测试脚本${NC}"
echo -e "${BLUE}========================================${NC}"
