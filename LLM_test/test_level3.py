...
# -*- coding: utf-8 -*-
"""
Level 3 Benchmark测试器 - 统一版本（支持zero-shot和few-shot）
调用API测试benchmark数据集，使用无窗口MATLAB验证，提取并运行LLM生成的MATPOWER代码，验证results.f的正确性
Level 3需要生成完整的MATPOWER脚本代码（包含结构性修改，如DCOPF）
"""
...
class Level3_BenchmarkTester:
    """
    Level 3 Benchmark测试器（支持zero-shot和few-shot）
    """
    ...
    def extract_matlab_code(self, llm_output: str) -> Optional[str]:
        ...
        for pattern in patterns:
            ...
            if matches:
                ...
                # 验证是否包含必要的MATPOWER元素
                if 'define_constants' in code or 'loadcase' in code:
                    return code
        
        # 如果没有代码块，尝试直接查找define_constants开始的代码
        if 'define_constants' in llm_output:
            lines = llm_output.split('\n')
            code_lines = []
            in_code = False
            
            for line in lines:
                if 'define_constants' in line:
                    in_code = True
...
Based on the following natural language description, generate a complete, executable MATLAB script that implements the described optimal power flow analysis with structural modifications.
...
4. Apply all parameter modifications as described:
   - Modify bus parameters (PD, QD, VMAX, VMIN, etc.) using proper indexing
   - Modify generator parameters (PMAX, PMIN, QMAX, QMIN, etc.) 
   - Modify branch parameters (BR_X, BR_R, RATE_A, etc.)

5. Handle structural modifications :
   - Set problem type: DC OPF or AC OPF using `mpoption('model', 'DC')` or `'AC'`
   - If custom objective functions are needed (e.g., angle difference penalty, voltage deviation penalty, reactive power cost):
     * Use `om = opf_setup(mpc, mpopt)` to set up the optimization model
     * Use `om.add_quad_cost()` to add quadratic cost terms
   - If additional constraints are needed (e.g., reserve requirements, N-1 security):
     * Use `om.add_var()` to add new variables
     * Use `om.add_lin_constraint()` to add linear constraints
   - Solve using `[x, fval, eflag, ~, ~] = om.solve()`

6. Configure solver options as specified:
   - Always set opf.violation if specified in the description
   - Example: `mpopt = mpoption('opf.violation', 1e-8, 'model', 'DC');`

7. Extract and store the final results in a `results` structure:
   - Extract solution variables from x vector using indices
   - Store objective value: `results.f = fval;`
   - Store convergence status: `results.success = (eflag > 0);`
   - Store bus results (VA, VM if AC) and generator results (PG, QG if AC)

8. The code should be ready to run without any modifications

**IMPORTANT:** 
- Generate complete, executable code only - no explanations or additional text
- Do not use placeholders or comments like "% apply modifications here"
- Include all necessary matrix operations for custom costs/constraints

Output the complete MATLAB script directly:
"""
    ...
    def run_matlab_code(
        ...
        matlab_code: str,
        sample_id: int
    ) -> tuple[str, Optional[float], Optional[bool], str]:
        """
        在MATLAB中运行代码并获取results.f和results.success
        
        Args:
            matlab_code: MATLAB代码
            sample_id: 样本ID
        ...
        if not self.matlab_engine:
            return "error", None, None, "MATLAB引擎未启动"
        ...
        try:
            ...
            # 获取results.f和results.success的值
            f_value = self.matlab_engine.eval("results.f", nargout=1)
            
            # 尝试获取success值
            try:
                success_value = self.matlab_engine.eval("results.success", nargout=1)
                success_value = bool(success_value)
            except:
                success_value = None
            
            if self.verbose:
                success_str = f", success={success_value}" if success_value is not None else ""
                print(f"  ✅ 样本 {sample_id}: MATLAB执行成功 (f={f_value:.6f}{success_str})")
            
            return "success", float(f_value), success_value, ""
            ...
            return "error", None, None, error_msg
    ...
    def test_sample(
        ...
        """
        # 提取正确答案
        ground_truth_data = sample.get("results", {})
        ground_truth = ground_truth_data.get("objective_value")
        ground_truth_converged = ground_truth_data.get("converged")
        ...
        result = {
            ...
            "ground_truth": {
                "objective_value": ground_truth,
                "converged": ground_truth_converged
            },
            "model_output": {
        ...
        if ground_truth is not None:
            status, f_value, success_value, error = self.run_matlab_code(
                matlab_code, sample_id
            )
            
            if status == "success" and f_value is not None:
                result["model_output"]["objective_value"] = f_value
                result["model_output"]["success"] = success_value
                ...
                if is_correct:
                    ...
                    self.stats["incorrect"] += 1
                    if self.verbose:
                        relative_error = abs(f_value - ground_truth) / abs(ground_truth + self.epsilon)
                        print(f"     ⚠️ 结果不匹配:")
                        print(f"        标准答案: {ground_truth:.6f}")
                        print(f"        模型输出: {f_value:.6f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Level 3 Benchmark测试器 - 统一版本（支持zero-shot和few-shot）
...
示例用法:
  # Zero-shot模式测试（默认）
  python LLM_test/test_level3.py --output base_test_output/level3_test_{model}_output.jsonl --model deepseek-chat
  
  # Few-shot模式测试
  python LLM_test/test_level3.py --output base_test_output/level3_test_{model}_output.jsonl --model deepseek-chat --few-shot --example-file ProOPF_D/level3_data_example.jsonl
  
  # 指定输入文件和API配置
  python LLM_test/test_level3.py --output level3_test.jsonl --model deepseek-chat --input-file ProOPF_B/level3_with_labels.jsonl --api-key YOUR_API_KEY --base-url https://api.deepseek.com/v1
  
  # 指定处理范围（处理第1到第10个样本）
  python LLM_test/test_level3.py --output level3_test.jsonl --model deepseek-chat --start 1 --end 10
        """
    )
    
    parser.add_argument("--output", type=str, required=True,
                       help="输出文件名（将保存到results/level3目录下）")
    parser.add_argument("--model", type=str, default="deepseek-chat",
                       help="模型名称 (默认: deepseek-chat)")
    parser.add_argument("--input-file", type=str, default=None,
                       help="输入文件路径（默认: ProOPF_B/level3_with_labels.jsonl）")
    parser.add_argument("--few-shot", action="store_true",
                       help="启用few-shot模式（默认: zero-shot模式）")
    parser.add_argument("--example-file", type=str, default=None,
                       help="Few-shot示例文件路径（仅在--few-shot时使用，默认: ProOPF_D/level3_data_example.jsonl）")
    ...
    input_file = args.input_file if args.input_file else "ProOPF_B/level3_with_labels.jsonl"
    
    # 如果启用few-shot，设置示例文件路径
    example_file = None
    if args.few_shot:
        example_file = args.example_file if args.example_file else "ProOPF_D/level3_data_example.jsonl"
    ...
    # 输出文件路径（如果输出路径包含目录，则使用该目录；否则放到results/level3目录下）
    ...
    if os.path.dirname(output_template):
        ...
        # 否则放到默认目录
        output_file = os.path.join("../results/level3", output_template)
    
    # 创建测试器
    tester = Level3_BenchmarkTester(
        api_key=api_key,
        base_url=base_url,
    ...
    # 测试benchmark
    print(f"{'='*60}")
    print(f"🧪 Level 3 Benchmark测试器 ({'Few-shot' if args.few_shot else 'Zero-shot'}模式)")
    print(f"{'='*60}")
    ...
    if stats['total'] > 0:
        avg_time = elapsed_time / stats['total']
        print(f"⚡ 平均每样本: {avg_time:.1f} 秒")
