...
# -*- coding: utf-8 -*-
"""
Level 2 Benchmark测试器 - 统一版本（支持zero-shot和few-shot）
...
class Level2_BenchmarkTester:
    """
    Level 2 Benchmark测试器（支持zero-shot和few-shot）
    """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="""Level 2 Benchmark测试器 - 统一版本（支持zero-shot和few-shot）
...
示例用法:
  # Zero-shot模式测试（默认）
  python LLM_test/test_level2.py --output base_test_output/level2_test_{model}_output.jsonl --model deepseek-chat
  
  # Few-shot模式测试
  python LLM_test/test_level2.py --output base_test_output/level2_test_{model}_output.jsonl --model deepseek-chat --few-shot --example-file ProOPF_D/level2_data_example.jsonl
  
  # 指定输入文件和API配置
  python LLM_test/test_level2.py --output level2_test.jsonl --model deepseek-chat --input-file ProOPF_B/level2_with_labels.jsonl --api-key YOUR_API_KEY --base-url https://api.deepseek.com/v1
  
  # 指定处理范围（处理第1到第10个样本）
  python LLM_test/test_level2.py --output level2_test.jsonl --model deepseek-chat --start 1 --end 10
  
  # 使用本地模型测试
  python LLM_test/test_level2.py --output local_test.jsonl --local-model /path/to/model
        """
    )
    ...
    parser.add_argument("--input-file", type=str, default=None,
                       help="输入文件路径（默认: ProOPF_B/level2_with_labels.jsonl）")
    parser.add_argument("--few-shot", action="store_true",
                       help="启用few-shot模式（默认: zero-shot模式）")
    parser.add_argument("--example-file", type=str, default=None,
                       help="Few-shot示例文件路径（仅在--few-shot时使用，默认: ProOPF_D/level2_data_example.jsonl）")
    ...
    input_file = args.input_file if args.input_file else "ProOPF_B/level2_with_labels.jsonl"
    
    # 如果启用few-shot，设置示例文件路径
    example_file = None
    if args.few_shot:
        example_file = args.example_file if args.example_file else "ProOPF_D/level2_data_example.jsonl"
    ...
    if os.path.dirname(output_template):
        ...
        # 否则放到默认目录
        output_file = os.path.join("../results/level2", output_template)
    
    # 创建测试器
    tester = Level2_BenchmarkTester(
    ...
    # 测试benchmark
    print(f"{'='*60}")
    print(f"🧪 Level 2 Benchmark测试器 ({'Few-shot' if args.few_shot else 'Zero-shot'}模式)")
    print(f"{'='*60}")
